...
#ifdef _WIN32
    const fs::path ico = icon_dir / (title_id + ".ico");
    if (!png_to_ico(png, ico))
...
static void create_shortcut(EmuEnvState &emuenv, const std::string &app_path, const std::string &app_name) {
    ...
    // Set icon location
    const fs::path ico = icons_dir / (fs::path(app_path).stem().string() + ".ico");
    if (fs::exists(ico))
        psl->SetIconLocation(ico.wstring().c_str(), 0);
    else
    ...
    const fs::path shortcut = desktop / (app_name + ".desktop");
    ...
    const fs::path png_path = icons_dir / (fs::path(app_path).stem().string() + ".png");
    ...
    f << "[Desktop Entry]\n"
      ...
      << exe_path.string() << " -r " << app_path << "\n"
                                                    "Icon="
      << (fs::exists(png_path) ? png_path.string() : exe_path.string()) << "\n"
